...
MODELS = [
    ...
    ("Gemma-2-9B", "bf16 · HF/MPS", "32b7d3ceba2b", "google/gemma-2-9b-it", False),
]
...
out = {"models": []}
for name, eng, run, model_id, is_default in MODELS:
    ...
    kinds_present = set()
    for uid in uids[:1]:
        b0 = store.load_boundaries(model_id, uid)
        kinds_present = {b["kind"] for b in b0["boundary_map"]["boundaries"]}
    preds, probe_kind, probe_layer = step_predictions(
        store, model_id, kinds_present, depth_to_layer
    )
    print(f"  {name}: {len(preds)} step predictions ({probe_kind} L{probe_layer})",
          flush=True)
    ...
    for uid in uids:
        ...
        for t in rec.turns:
            ...
            turns.append({
                ...
                "predY": preds.get((uid, t.turn_index)),
            })
        bounds = [{
        ...
        samples.append({
            "seed": (rec.protocol or {}).get("seed", 0),
            ...
            for scope, idxs in scopes.items():
                ...
                for centered in (True, False):
                    ...
                    if scope in TSNE_SCOPES:
                        ...
                        if ures is not None:
                            uemb, nn = ures
                            uemb3 = np.column_stack([uemb, np.zeros(len(uemb))])
                            fits[f"{layer}|{scope}|{tag}|umap"] = {
                                "idx": idxs,
                                "xyz": norm_q(uemb3),
                                "nn": nn,
                            }
