...
SCENARIO_PATH="${REPO_ROOT}/configs/scenarios/eap_top_components_corrupted.yaml"
...
log() {
  printf '[run-eap-corrupted-scenario] %s\n' "$*"
}
...
run_corrupted_scenario() {
  if [[ ! -f "${SCENARIO_PATH}" ]]; then
    printf 'Missing EAP corrupted scenario config: %s\n' "${SCENARIO_PATH}" >&2
    exit 1
  fi

  cd "${REPO_ROOT}"
  log "Running EAP corrupted scenario ${SCENARIO_PATH#${REPO_ROOT}/}"
